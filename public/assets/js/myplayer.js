...
jQuery(document).ready(function ($) {
  ...
  adonisPlayer.playNext = function () {
    if (playingMode == 'shuffle') {
      ...
      if (currentPlayingMusicIndex == adonisAllPlaylists.length) {
        currentPlayingMusicIndex = 0;
      }
      adonisPlayer.play();
    } else {
      if (currentPlayingMusicIndex == adonisAllPlaylists.length - 1) {
        applemusic.player.stop();
      } else {
        currentPlayingMusicIndex++;
        adonisPlayer.play();
  ...
  adonisPlayer.updatePlaylists = function () {
    ...
    adonisAllPlaylists.forEach(music => {
      let playcss = '',
        playanimationcss = '';
      if (currentPlayingMusicIndex == index) {
        playcss = ' jp-playlist-current';
        playanimationcss = ' playing';
      }
