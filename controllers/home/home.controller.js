...
class HomeController {
    ...
    getPlaylists(req, res) {
        ...
        if (!music_user_token || !music_src) res.status(401).json({
            status: 'error',
    ...
    addMusicToPlaylist(req, res) {
        ...
        if (!music_id || !music_src || !music_type || !music_mode || !music_user_token) res.status(400).json({
            status: 'error',
            message: 'parameter error.'
        });
    ...
    async charts(req, res) {
        ...
        var url = '';
        if (userdata.mode == 'Apple') {
            url = apiconfigs.musickit_apiurl + `/getCatalogCharts`;
        } else {
            url = apiconfigs.spotify_apiurl + `/getRecommendations`;
        }
        fetch(url, {
                method: 'POST',
                body: JSON.stringify({
